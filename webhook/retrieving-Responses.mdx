...
- `amount` - the amount the deposit was for (applies only to `customer-deposit.successful` and `customer-deposit.failed`)
...
- `balance` - the current wallet balance (applies only to `balance.low` events)
