...
payload = {
        "customerId": "someuser@test.com",  // Let's assume your Agent system stores users unique by email address, 
                                            // you can then send this same value when creating a task to use these deposited funds

        "currencyCode": "USDCBASE",         // USDC on the Base network, use "USD" to request US Dollars instead.
